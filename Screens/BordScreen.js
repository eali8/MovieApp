...
export default function CheersScreen({ navigation }) {
  ...
  useEffect(() => {
    fetch('https://api.themoviedb.org/3/discover/movie?api_key=744cbd8d140536377ac58c1f939dd82d&with_genres=28')
      .then(res => res.json())
      .then(
        (result) => {
          setIsLoaded(true);
  ...
  const renderRandomComedyMovie = () => {
    if (dataResult.length > 0) {
      const comedyMovies = dataResult.filter(item => item.genre_ids.includes(28)); 
