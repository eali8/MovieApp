...
export default function CheersScreen({ navigation }) {
  ...
  useEffect(() => {
    fetch('https://api.themoviedb.org/3/discover/movie?api_key=af117673b0468e9ebcee2742e751f872&with_genres=9648')
      .then(res => res.json())
      .then(
        (result) => {
          setIsLoaded(true);
  ...
  const renderRandomComedyMovie = () => {
    if (dataResult.length > 0) {
      const comedyMovies = dataResult.filter(item => item.genre_ids.includes(9648)); //
